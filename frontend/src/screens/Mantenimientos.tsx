...
export default function Mantenimientos() {
  ...
  const list = useQuery({
    ...
    queryFn: async () => {
      const res = search
        ? await MantenimientosAPI.search({
            q: search,
            order: "asc",
            offset: 0,
            limit: 50,
          })
        : await MantenimientosAPI.getAll();
      return ensureArray<Mantenimiento>(res.data);
    },
  });

  const [form, setForm] = useState<CrearMantenimiento>({
    consecutivo_orden: 0,
    tipo: "Preventivo",
    descripcion: "",
  ...
  return (
    <section className="space-y-4">
      <div className="card">
        <h1 className="page">Mantenimientos</h1>
        <p className="text-coffee-700">Búsqueda y CRUD</p>
      </div>
      ...
      <div className="card flex gap-2">
        ...
        <button
          ...
          onClick={() =>
            qc.invalidateQueries({ queryKey: ["mantenimientos", search] })
          }
        >
          Buscar
        </button>
      ...
      <div className="card grid md:grid-cols-5 gap-2">
        <input
          className="input"
          placeholder="Consecutivo Orden"
          value={form.consecutivo_orden}
          onChange={(e) =>
            setForm({ ...form, consecutivo_orden: Number(e.target.value) })
          }
        />
        <input
          className="input"
        ...
        <button
          ...
          disabled={!form.consecutivo_orden || !form.descripcion}
        >
          Crear
        </button>
      </div>
            ...
            <tbody>
              {(list.data ?? []).map((m: Mantenimiento) => (
                <tr key={m.numero} className="border-t">
                  <td>{m.numero}</td>
                  <td>{m.consecutivo_orden}</td>
                  <td>{m.tipo ?? "—"}</td>
                  <td>{m.descripcion}</td>
                  <td className="text-right">
                    <button
                      className="btn btn-ghost"
                      onClick={() => remove.mutate(m.numero)}
                    >
                      Eliminar
                    </button>
                  </td>
